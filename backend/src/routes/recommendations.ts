...
router.post('/generate-itinerary', authenticate, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    const { duration, startTime, interests, budget } = req.body;

    if (!duration || !startTime || !interests || budget === undefined) {
      return res.status(400).json({
        error: 'Duration, startTime, interests, and budget are required'
      });
    }
